...
export default function RecruiterLayout({ children, title }) {
  ...
  const isDash = location.pathname === '/recruiter/dashboard';
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <aside className={`fixed lg:static top-0 left-0 z-40 h-screen bg-white border-r border-slate-100 flex flex-col shrink-0 transition-all duration-300 ease-in-out overflow-hidden ${open ? 'w-60' : 'w-0'}`}>
          ...
          <div className="px-5 py-5 border-b border-slate-100">
            <div className="flex items-center gap-2.5 mb-2">
              ...
              <span className="font-display font-bold text-xl tracking-tight">
                <span className="text-blue-600">Hire</span><span className="text-slate-900">Smart</span>
              </span>
            </div>
            <span className="badge badge-blue text-xs">Recruiter Portal</span>
          </div>

          <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider px-3 mb-2">Menu</p>
            <NavLink to="/recruiter/dashboard" end className={({ isActive }) => `sidebar-link${isActive ? ' active' : ''}`}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-slate-100 shrink-0 sticky top-0 z-20">
          <div className="flex items-center gap-3 px-5 h-14">
            <button onClick={() => setOpen(o => !o)} className="w-8 h-8 rounded-lg flex flex-col items-center justify-center gap-[5px] hover:bg-slate-100 transition-colors shrink-0">
              <span className="block w-[16px] h-[1.5px] bg-slate-600 rounded" />
              <span className="block w-[16px] h-[1.5px] bg-slate-600 rounded" />
              <span className="block w-[16px] h-[1.5px] bg-slate-600 rounded" />
            </button>
            {!isDash && (
              <button onClick={() => navigate(-1)} className="flex items-center gap-1 text-sm font-semibold text-slate-500 hover:text-slate-900 transition-colors">
